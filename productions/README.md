# Four formal package production fixtures
...
variants reuse the same language/Hook production input. Retries verify and reuse that manifest,
rather than calling either provider again. Only sanitized brief fields and asset metadata are sent to
DeepSeek; order, payment, secret, URL, and local-path fields are excluded.

The current runtime profile contains absolute macOS paths for FFmpeg, FFprobe, and the locally
cached Chrome binary. Re-run or generate a machine-local profile before moving the checkout.
...
vendor/hypit/hypit get <build-id> \
  --output final.video \
  --to productions/previews/final/<package>.mp4 \
  --workspace productions --json
```
...
- `validateCampaignDeliverables` accepted the exact language, hook, aspect ratio, duration, and file
  count for all four package fixtures. The current validator also confirmed usable audio and zero
...
- The repository syntax check and all automated tests passed.
...
Docker worker isolation test also passed, so all four workflows are marked `readyForSale: true` for
the Hackathon Demo. Exhaustive rendering of every add-on combination is outside the agreed Demo
