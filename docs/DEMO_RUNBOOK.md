# Three-minute preview walkthrough

This runbook reflects the 2026-09 Demo Day presentation and remains a preview walkthrough.
Check the current GoBTC status before any new presentation; describe the provider response used in
that session and distinguish a simulated payment from a verified live payment.

1. Start on the Hirebit console. For the historical Demo Day flow, the organizers confirmed the GoBTC outage and approved disclosed simulated responses. For a new walkthrough, state the payment mode actually in use.
...
6. In preview mode, identify the step as **Simulated payment authorization**. Show that the simulated invoice and fee together stay within the displayed maximum. Explain that mandate, reservation, order and production gates run with a simulated provider response. Mark the BTC transfer as simulated and chain settlement as pending. Keep private keys and API tokens out of view.
...
8. Open the delivered video and summarize the package, total spend and remaining budget. In preview mode, identify the payment as simulated; present a chain transaction ID only when one has been verified.
